  ...
  return (
    <div className="space-y-6 animate-fade-in text-slate-900 dark:text-slate-100 font-sans">
      ...
      <Panel dark title={`Resource Batches (${filtered.length})`}>
        {loading ? (
          <div className="p-8 text-center text-xs text-slate-500 dark:text-slate-400">Loading inventory...</div>
        ) : filtered.length === 0 ? (
          <EmptyState icon={Package}>
            <p className="font-semibold text-slate-600 dark:text-slate-300">No resources found</p>
          </EmptyState>
        ) : (
          <Table columns={["Batch ID", "Type", "Quantity", "Pickup Zone", "Expiry Window", "Status"]}>
            {filtered.map((item, idx) => (
              <tr key={item.id || item.resource_id || `resource-${idx}`} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                <td className="px-4 py-3 font-mono font-bold text-sky-600 dark:text-sky-400">
                  {item.id || item.resource_id || "RES-001"}
                </td>
                <td className="px-4 py-3 font-semibold text-slate-900 dark:text-white">{item.resource_type}</td>
                <td className="px-4 py-3 text-slate-700 dark:text-slate-300">{item.quantity} units</td>
