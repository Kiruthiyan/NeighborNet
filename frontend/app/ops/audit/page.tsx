...
import { Panel, StatusBadge, EmptyState, Table, Input, Select } from "../../../components/ui";
  ...
  return (
    <div className="space-y-6 animate-fade-in text-slate-900 dark:text-slate-100 font-sans">
      ...
      <Panel dark title={`Audit Event History (${filtered.length})`}>
        {loading ? (
          <div className="p-8 text-center text-xs text-slate-500 dark:text-slate-400">Loading audit history...</div>
        ) : filtered.length === 0 ? (
          <EmptyState icon={FileText}>
            <p className="font-semibold text-slate-600 dark:text-slate-300">No matching audit events</p>
          </EmptyState>
        ) : (
          <Table columns={["Timestamp", "Actor / Agent", "Action Description", "Operating Mode", "Risk Tier", "Human Approval"]}>
            {filtered.map((log, idx) => (
              <tr key={log.id || idx} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                ...
                <td className="px-4 py-3 text-slate-700 dark:text-slate-200">{log.action || log.event_type}</td>
