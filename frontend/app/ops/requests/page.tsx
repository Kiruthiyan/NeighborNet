...
import { HeartHandshake, Filter, Search, Globe2, AlertTriangle } from "lucide-react";
import { Panel, Table, StatusBadge, EmptyState, Badge, Input, Select, Button } from "../../../components/ui";
    ...
    <div className="space-y-6 animate-fade-in text-slate-900 dark:text-slate-100 font-sans">
      <div>
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight flex items-center gap-2">
          <HeartHandshake className="text-sky-600 dark:text-sky-400" size={24} />
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
        <div className="relative flex-1 w-full">
          <Search className="absolute left-3 top-2.5 text-slate-400 dark:text-slate-500 pointer-events-none" size={16} />
          <Input
            type="text"
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          <Filter className="text-slate-400 dark:text-slate-500 shrink-0" size={16} />
          <Select
            value={statusFilter}
      ...
      <Panel dark title="Cross-Region Assistance Check" icon={Globe2}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 items-end">
          <div>
            <label className="block text-[11px] font-bold uppercase text-slate-500 dark:text-slate-400 mb-1">Region in need</label>
            <Select
              value={crRegion}
          ...
          <div>
            <label className="block text-[11px] font-bold uppercase text-slate-500 dark:text-slate-400 mb-1">Resource type</label>
            <Select
          ...
          <div>
            <label className="block text-[11px] font-bold uppercase text-slate-500 dark:text-slate-400 mb-1">Quantity needed</label>
            <Input
              type="number"
              min={0}
              value={crQuantity}
          ...
          <Button onClick={handleCheckCrossRegion} loading={crLoading} variant="primary">
            {!crLoading && <Globe2 size={14} />}
            Check
          </Button>
        </div>

        {crError && (
          <div className="mt-3 flex items-center gap-2 rounded-lg bg-rose-50 dark:bg-rose-950/60 border border-rose-200 dark:border-rose-500/40 p-3 text-xs text-rose-700 dark:text-rose-300">
            <AlertTriangle size={14} /> {crError}
          </div>
        )}

        {crResult && (
          <div className="mt-4 p-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 text-xs text-slate-700 dark:text-slate-300 space-y-2">
            <p>
              Local available in <strong className="text-slate-900 dark:text-white">{crResult.region}</strong>:{" "}
              <strong className="text-slate-900 dark:text-white">{crResult.local_available}</strong> — Shortfall:{" "}
              <strong className={crResult.shortfall > 0 ? "text-rose-600 dark:text-rose-400" : "text-emerald-600 dark:text-emerald-400"}>
                {crResult.shortfall}
              </strong>
            </p>
            {crResult.shortfall === 0 ? (
              <p className="text-emerald-600 dark:text-emerald-400">Local supply covers the need — no cross-region assistance required.</p>
            ) : crResult.cross_region_candidates.length === 0 ? (
              <p className="text-amber-600 dark:text-amber-400">No other region currently has surplus to offer.</p>
            ) : (
      ...
      <Panel dark title={`All Community Requests (${filtered.length})`}>
        {loading ? (
          <div className="p-8 text-center text-xs text-slate-500 dark:text-slate-400">Loading requests...</div>
        ) : filtered.length === 0 ? (
          <EmptyState icon={HeartHandshake}>
            <p className="font-semibold text-slate-600 dark:text-slate-300">No matching requests found</p>
          </EmptyState>
        ) : (
          <Table columns={["Request ID", "Resource Type", "Qty", "Urgency", "Status", "Created"]}>
            {filtered.map((req) => (
              <tr key={req.id || req.request_id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                <td className="px-4 py-3 font-mono font-bold text-sky-600 dark:text-sky-400">
                  {req.request_id || req.id || "REQ-001"}
                </td>
                <td className="px-4 py-3 font-semibold text-slate-900 dark:text-white">{req.resource_type}</td>
                <td className="px-4 py-3 text-slate-700 dark:text-slate-300">{req.quantity}</td>
                <td className="px-4 py-3">
                  <span
                    className={`text-[11px] font-bold ${
                      req.urgency === "HIGH" ? "text-rose-600 dark:text-rose-400" : "text-slate-500 dark:text-slate-400"
                    }`}
                  >
                    {req.urgency || "MEDIUM"}
                  </span>
