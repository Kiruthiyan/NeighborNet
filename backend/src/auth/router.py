...
def _dev_otp(otp: str) -> Optional[str]:
    """Only surface the plaintext OTP in an API response when BOTH email
    delivery isn't configured AND the server is explicitly running in debug/
    non-production mode - dev/demo fallback, mirrors
    InvitationResponse.email_sent. A production deployment that simply never
    configures SMTP must fail closed here (OTP goes nowhere, delivery must be
    fixed) rather than leak the code to whoever called the endpoint - that
    would defeat the OTP's purpose as proof of email ownership."""
...
async def login(payload: LoginRequest) -> TokenResponse:
    enforce_rate_limit(f"login:{payload.email.strip().lower()}", max_attempts=10, window_seconds=300)
    service = get_coordination_service()
...
async def verify_email(payload: VerifyEmailRequest) -> UserProfile:
    ...
    enforce_rate_limit(f"verify_email:{payload.email.strip().lower()}", max_attempts=5, window_seconds=300)
    service = get_coordination_service()
    user = service.get_user_by_email(payload.email)
    if user is None:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid or expired code")
...
async def resend_verification(payload: EmailOnlyRequest) -> ForgotPasswordResponse:
    enforce_rate_limit(f"resend_verification:{payload.email.strip().lower()}", max_attempts=5, window_seconds=300)
    service = get_coordination_service()
    user = service.get_user_by_email(payload.email)
...
async def forgot_password(payload: EmailOnlyRequest) -> ForgotPasswordResponse:
    enforce_rate_limit(f"forgot_password:{payload.email.strip().lower()}", max_attempts=5, window_seconds=300)
    service = get_coordination_service()
    user = service.get_user_by_email(payload.email)
    if user is not None:
...
async def reset_password(payload: ResetPasswordRequest) -> UserProfile:
    enforce_rate_limit(f"reset_password:{payload.email.strip().lower()}", max_attempts=5, window_seconds=300)
    service = get_coordination_service()
    user = service.get_user_by_email(payload.email)
    if user is None:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid or expired code")
